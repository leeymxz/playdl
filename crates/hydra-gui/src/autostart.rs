...
//! "Startup apps" page show the icon and "PlayDL Download Manager" name
...
fn entry_path() -> Option<PathBuf> {
    Some(dirs::home_dir()?.join(".config/autostart/playdl.desktop"))
}
...
fn apply_platform(enabled: bool, minimized: bool) {
    ...
    const RUN_VALUE: &str = "PlayDL Download Manager";

    // Versions before 0.2.x wrote a playdl.cmd into the Startup folder, which
    ...
    if let Some(legacy) = dirs::config_dir()
        .map(|d| d.join("Microsoft/Windows/Start Menu/Programs/Startup/playdl.cmd"))
    {
        if legacy.exists() && std::fs::remove_file(&legacy).is_ok() {
            crate::log::info("legacy Startup-folder playdl.cmd removed");
        }
    }
...
fn apply_platform(enabled: bool, minimized: bool) {
    ...
    if !enabled {
        // On Linux the deb/rpm packages ship /etc/xdg/autostart/playdl.desktop,
...
mod tests {
    ...
    fn installed_paths_pass_through() {
        let p = PathBuf::from(
            "/Applications/PlayDL Download Manager.app/Contents/MacOS/PlayDL Download Manager",
        );
    ...
    fn transient_paths_without_an_installed_copy_are_rejected() {
        for p in [
            "/private/var/folders/ab/xyz/T/AppTranslocation/1234/d/NoSuchPlayDL.app/Contents/MacOS/NoSuchPlayDL",
            "/Volumes/PlayDL/NoSuchPlayDL.app/Contents/MacOS/NoSuchPlayDL",
        ] {
